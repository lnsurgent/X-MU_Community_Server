...
void CAccountCharDBSet::SetVipStatus(LPSTR AccountID, int Duration, int vipType)
{
	CString qSQL;
	qSQL.Format("UPDATE MEMB_INFO SET vip = %d, VipDuration = VipDuration + %d WHERE memb___id='%s'", vipType, Duration, AccountID);
...
void CAccountCharDBSet::UpdateVipStatus(int Duration)
{
	CString qSQL;
	qSQL.Format("UPDATE MEMB_INFO SET VipDuration = VipDuration - %d WHERE VipDuration > 0 and vip > 0", Duration);
...
void CAccountCharDBSet::RemoveVipStatus(int value)
{
	CString qSQL;
	qSQL.Format("SELECT * FROM MEMB_INFO");
	int vipDur = 0;
	...
	vipDur = m_DBQuery.GetInt("VipDuration");

	if(vipDur == 0)
	{
		m_DBQuery.Clear();
		qSQL.Format("UPDATE MEMB_INFO SET vip = %d WHERE VipDuration = 0",value);
		if(m_DBQuery.Exec(qSQL) == FALSE)
		{
			m_DBQuery.Clear();
			return;
...
BOOL CAccountCharDBSet::GetVipInfo(LPSTR AccountId, int aIndex)
{
	CString qSQL;
	bool retorno = false;
	int vip = 0;
	qSQL.Format("SELECT * FROM MEMB_INFO WHERE memb___id = '%s'",AccountId);
...
BOOL CAccountCharDBSet::CheckVipStats(char * AccountId)
{
	CString qSQL;
	bool retorno = false;
	int vip = 0;
	qSQL.Format("SELECT vip FROM MEMB_INFO WHERE memb___id = '%s'",AccountId);
...
int CAccountCharDBSet::GetVipType(char* szAccountId)
{
	CString qSQL;
	bool retorno = false;
	int vip = 0;
	qSQL.Format("SELECT Vip FROM MEMB_INFO WHERE memb___id = '%s'",szAccountId);
