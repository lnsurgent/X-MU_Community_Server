...
void ProtocolCore(BYTE protoNum, BYTE *aRecv, int aLen, short aIndex)
{
	...
	/*char proto[22];
	sprintf(proto, " %d ", protoNum);
	gWindow.PrintLog(proto, __FILE__, __LINE__);*/
	switch(protoNum)
	{
	case 0:
	...
	case 0xD7:	//Custom
		{
			...
			case 0x01:
				{
					UPDATEGOLDCHANNELSTATUS * pReq = (UPDATEGOLDCHANNELSTATUS*)aRecv;
					gACDbSet.SetVipStatus(pReq->AccountID, pReq->Duration, pReq->vipType);
				}break;
			case 0x02:
				{
					UPDATEVIPSTATUS * pReq = (UPDATEVIPSTATUS*)aRecv;
					gACDbSet.UpdateVipStatus(pReq->Duration);
				}break;
			case 0x03:
				{
					REMOVEVIPSTATUS * pReq = (REMOVEVIPSTATUS*)aRecv;
					gACDbSet.RemoveVipStatus(pReq->Value);
				}break;
			case 0x04:
				{
					GETVIPSTATUS * pReq = (GETVIPSTATUS*)aRecv;
					if(gACDbSet.GetVipInfo(pReq->AccountID, pReq->aIndex))
					{
						SENDVIPSTATUS pMsg;
						pMsg.h.set((LPBYTE)&pMsg, 0xDD, 0x01, sizeof(SENDVIPSTATUS));
						pMsg.aIndex = pReq->aIndex;

						wsjServer.DataSend(aIndex, (char*)&pMsg, sizeof(SENDVIPSTATUS));
					}
				}break;
			case 0x05:
				{
					CHECKVIP * pReq = (CHECKVIP*)aRecv;
					if(!gACDbSet.CheckVipStats(pReq->AccountId))
					{
						SENDDISCONNECTVIP pMsg;
						pMsg.h.set((LPBYTE)&pMsg, 0xDD, 0x02, sizeof(SENDDISCONNECTVIP));
						pMsg.aIndex = pReq->aIndex;
						wsjServer.DataSend(aIndex, (char*)&pMsg, sizeof(SENDDISCONNECTVIP));
					}
				}break;
				case 0x06:
				{
					...
					SENDVIPTYPE pMsg;
					pMsg.h.set((LPBYTE)&pMsg, 0xDD, 0x03, sizeof(SENDVIPTYPE));
					pMsg.aIndex = pReq->aIndex;
					pMsg.Vip = gACDbSet.GetVipType(pReq->AccountID);

					wsjServer.DataSend(aIndex, (char*)&pMsg, sizeof(SENDVIPTYPE));
...
void GJPSetAccountInfo(LPSDHP_REQ_SETACCOUNTINFO lpMsg, int aIndex)
{
	...
	if(lpMsg->type == 1)
		...
		Result.Result = gACDbSet.SetRageFighterCardInfo(lpMsg->AccountId);
